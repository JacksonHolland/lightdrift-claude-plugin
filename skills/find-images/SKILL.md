...
Use the Lightdrift MCP tools exposed by this plugin: `search_images`, `find_similar_images`, and `get_image`. Discover their current schemas rather than guessing parameter names or response fields. If both the plugin and a standalone Lightdrift connector are available, prefer the plugin’s namespaced tools and use only one connection for each operation. Do not duplicate paid calls across connections.
...
On an authentication error, reconnect rather than looping. On insufficient credit, explain the balance issue and stop paid calls. On rate limiting, respect the retry delay. On a temporary search availability error, respect Retry-After and retry at most once when the budget permits. The current search pipeline does not require GPU warmup. If no result matches, say so rather than substituting invented imagery or attribution.
...
For hosted Claude downloads, a domain allowlist may need `api.lightdrift.ai` plus Lightdrift’s storage hostname `26b17c22f73aeeffe81bee6419afe7ad.r2.cloudflarestorage.com`. If blocked, explain how to add those exact domains under Settings → Capabilities → Additional allowed domains. Do not bypass the sandbox or ask for unrestricted network access.
