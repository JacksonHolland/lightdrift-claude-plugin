...
EXPERIMENT = "lig103_presentation_search_v1"
...
def main(argv=None, opener=None):
    ...
    parser.add_argument("topics", nargs=3, help="Three quoted slide image briefs")
    parser.add_argument("--dry-run", action="store_true", help="Print request bodies; no network or key needed")
    args = parser.parse_args(argv)
    topics = [s.strip() for s in args.topics]
    if any(not s or len(s) > 1000 for s in topics):
        parser.error("Each topic must contain 1–1000 characters")
    key = os.environ.get("LIGHTDRIFT_API_KEY", "").strip()
    if not args.dry_run and not key:
        parser.error("Set LIGHTDRIFT_API_KEY in your environment")
    session = str(uuid.uuid4())
    opener = opener or urllib.request.build_opener(NoRedirect())
    for slide, topic in enumerate(topics, 1):
        body = payload(topic, session)
        if args.dry_run:
            print(json.dumps({"slide": slide, "method": "POST", "url": ENDPOINT, "body": body}))
            continue
        ...
        print(json.dumps({"slide": slide, "topic": topic, "response": data}), flush=True)
    return 0
